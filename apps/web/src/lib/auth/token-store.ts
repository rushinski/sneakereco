...
const channel =
  typeof window !== 'undefined' ? new BroadcastChannel('sneakereco-auth') : null;

type AuthMessage =
  | { type: 'set'; accessToken: string; expiresAt: number }
  | { type: 'clear' };
...
export function subscribeToAuthChanges(
  ...
  const handler = (e: MessageEvent<AuthMessage>) => {
    if (e.data.type === 'set') callback(e.data.accessToken);
    if (e.data.type === 'clear') callback(null);
  };
