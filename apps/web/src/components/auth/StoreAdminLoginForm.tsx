...
export function StoreAdminLoginForm() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const csrfData = await apiClient.getCsrfToken('store-admin');
        if (!cancelled) {
  ...
  const completeLogin = (accessToken: string) => {
    ...
    router.push(getStoreAdminExternalPath('/admin', window.location.host));
  };
  ...
  async function handleLoginSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await apiClient.loginStoreAdmin({ email, password }, csrfToken);
  ...
  async function handleMfaSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await apiClient.completeStoreAdminMfaChallenge(
  ...
  async function handleMfaSetupSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await apiClient.completeStoreAdminMfaSetup(
        ...
        <div className="w-full max-w-md rounded-2xl border border-gray-200 bg-white px-8 py-10 shadow-sm">
          ...
          <div className="mt-6 flex gap-6 flex-wrap">
            ...
            <form
              ...
              onSubmit={(event) => {
                void handleMfaSetupSubmit(event);
              }}
            >
              <AuthField
