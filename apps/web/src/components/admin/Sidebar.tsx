...
export function Sidebar({ tenantName }: { tenantName: string }) {
  ...
  const dashboardHref = getStoreAdminExternalPath(
    '/admin',
    typeof window === 'undefined' ? '' : window.location.host,
  );
  const webDesignHref = getStoreAdminExternalPath(
    '/admin/web-design',
    typeof window === 'undefined' ? '' : window.location.host,
  );
  ...
  async function handleLogout() {
    try {
      ...
      const csrf = readCsrfTokenCookie('store-admin');
      if (token && csrf) {
        await apiClient.logoutStoreAdmin(csrf, token).catch(() => {});
      }
    } finally {
      clearAccessToken();
      router.replace(
    ...
    <aside className="relative z-10 flex h-screen w-60 shrink-0 flex-col bg-gray-900">
      ...
      <div className="flex items-center gap-3 border-b border-gray-800 px-4 py-5">
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-gray-700">
          <span className="text-xs font-bold uppercase text-gray-400">{tenantName.charAt(0)}</span>
        </div>
      ...
      <nav className="flex-1 space-y-0.5 overflow-y-auto px-3 py-4">
        <NavItem href={dashboardHref} label="Dashboard" icon={<IconDashboard />} />
        <NavItem href={webDesignHref} label="Web Design" icon={<IconWebDesign />} />
      </nav>
