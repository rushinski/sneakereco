...
export class PasswordResetService {
  ...
  async forgotPassword(dto: ForgotPasswordDto, pool: PoolCredentials): Promise<{ success: true }> {
    await this.cognito.forgotPassword(dto, pool);
    return { success: true };
  }

  async resetPassword(dto: ResetPasswordDto, pool: PoolCredentials): Promise<{ success: true }> {
