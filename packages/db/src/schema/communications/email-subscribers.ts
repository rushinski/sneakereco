...
export const emailSubscriberStatusValues = ['pending', 'confirmed', 'unsubscribed'] as const;

export type EmailSubscriberStatus = (typeof emailSubscriberStatusValues)[number];
...
export const emailSubscribers = pgTable(
  'email_subscribers',
  {
    ...
    source: text('source'),
    status: text('status', { enum: emailSubscriberStatusValues }).notNull().default('pending'),
  ...
  (table) => [
    uniqueIndex('uniq_email_subscribers_tenant_email').on(table.tenantId, table.email),
