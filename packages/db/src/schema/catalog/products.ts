...
export const products = pgTable(
  ...
  (table) => [
    ...
    index("idx_products_tenant_active").on(
      table.tenantId,
      table.isActive,
      table.goLiveAt.desc(),
    ),
    ...
    check(
      "products_condition_check",
      sql`${table.condition} in ('new', 'preowned')`,
    ),
    check("products_cost_cents_check", sql`${table.costCents} >= 0`),
    check(
      "products_sku_format_check",
      sql`${table.sku} ~ '^[A-Z]{2,4}-[A-Z]{2,4}-\\d{5}$'`,
    ),
  ],
);
