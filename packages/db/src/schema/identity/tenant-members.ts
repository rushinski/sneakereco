...
export const tenantMembers = pgTable(
  'tenant_members',
  {
    ...
    role: text('role', { enum: tenantMemberRoleValues }).notNull().default('customer'),
    isOwner: boolean('is_owner').notNull().default(false),
    isOrderEmailsEnabled: boolean('is_order_emails_enabled').notNull().default(true),
    createdAt: createdAtColumn(),
    updatedAt: updatedAtColumn(),
  },
  (table) => [
    uniqueIndex('uniq_tenant_members_tenant_user').on(table.tenantId, table.userId),
    ...
    check('tenant_members_role_check', sql`${table.role} in ('customer', 'admin')`),
  ],
);
