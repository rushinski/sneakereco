...
export const nexusRegistrationsAdminManagePolicy = pgPolicy('nexus_registrations_admin_manage', {
  for: 'all',
...
export const stateSalesTrackingAdminReadPolicy = pgPolicy('state_sales_tracking_admin_read', {
  for: 'select',
  to: sneakerecoAppRole,
  using: tenantAdminScope(stateSalesTracking.tenantId),
}).link(stateSalesTracking);

export const tenantTaxSettingsAdminManagePolicy = pgPolicy('tenant_tax_settings_admin_manage', {
  for: 'all',
  to: sneakerecoAppRole,
